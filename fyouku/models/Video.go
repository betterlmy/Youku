...
func GetChannelHotList(channelId int) []Video {
	var info []Video
	req := httplib.Get(getApiURL() + "/channel/hot?channelId=" + strconv.Itoa(channelId))
...
func GetChannelTypeRecommend(channelId int, typeId int) []Video {
	var info []Video
	req := httplib.Get(getApiURL() + "/channel/recommend/type?channelId=" + strconv.Itoa(channelId) + "&typeId=" + strconv.Itoa(typeId) + "")
	str, err := req.String()
	if err != nil {
		fmt.Println(err)
	}
...
func GetChannelRegionRecommend(channelId int, regionId int) []Video {
	var info []Video
	req := httplib.Get(getApiURL() + "/channel/recommend/region?channelId=" + strconv.Itoa(channelId) + "&regionId=" + strconv.Itoa(regionId) + "")
	str, err := req.String()
	if err != nil {
		fmt.Println(err)
	}
...
func GetChannelVideoList(channelId int, regionId int, typeId int, end string, sort string, page int) ([]Video, int64) {
	var info []Video
	var count int64
	url := getApiURL() + "/channel/video?channelId=" + strconv.Itoa(channelId)
	if regionId > 0 {
...
func GetSearchVideoList(keyword string, page int) ([]Video, int64) {
	...
	req := httplib.Post(getApiURL() + "/video/search")
...
func GetMyVideos(uid int) string {
	req := httplib.Get(getApiURL() + "/user/video?uid=" + strconv.Itoa(uid) + "")
	str, err := req.String()
	if err != nil {
		fmt.Println(err)
	}
...
func GetVideoInfo(videoId int) VideoInfo {
	req := httplib.Get(getApiURL() + "/video/info?videoId=" + strconv.Itoa(videoId) + "")
	str, err := req.String()
	if err != nil {
		fmt.Println(err)
	}
...
func GetVideoEpisodesList(videoId int) []Episodes {
	req := httplib.Get(getApiURL() + "/video/episodes/list?videoId=" + strconv.Itoa(videoId) + "")
	str, err := req.String()
	if err != nil {
		fmt.Println(err)
	}
...
func GetChannelAdvert(channelId int) []Advert {
	var info []Advert
	req := httplib.Get(getApiURL() + "/channel/advert?channelId=" + strconv.Itoa(channelId) + "")
...
func SaveVideoInfo(uid int, playUrl string, title string, subTitle string, channelId int, typeId int, regionId int, aliyunVideoId string) string {
	req := httplib.Post(getApiURL() + "/video/save")
